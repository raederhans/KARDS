...
export const UI_TEXT = {
  en: {
    ...
    help: {
      ...
      intro: "Build the card in the editor, use Undo or Redo when needed, then use the workspace to reference, save, or export it.",
      ...
      workspaceItems: [
        { title: "Appearance", body: "Apply one of five serialized appearance presets, adjust paper texture, or load a local style pack." },
        ...
        { title: "Reference", body: "Search or apply bundled reference cards, and locate visual differences against an image you choose." },
      ],
      ...
      tips: [
        "Outside text fields, use Ctrl/Cmd+Z to undo and Ctrl/Cmd+Shift+Z or Ctrl+Y to redo. Text fields keep their native text undo.",
        "Title and body fitting starts from your manual size and scale settings. The status below each field shows the resolved size and warns when text will still be omitted at the minimum size.",
        "The Canvas preview has a readable card summary. Use the Artwork crop number fields or sliders for keyboard control.",
        "Visual-difference levels, thresholds, and coordinates are review signals, not pass/fail results.",
    ...
    fieldPanel: {
      ...
      textFitFits: (fontSize: number) => `Automatic fit: the requested ${fontSize} px fits.`,
      textFitAdjusted: (requestedFontSize: number, resolvedFontSize: number) => `Automatic fit reduced ${requestedFontSize} px to ${resolvedFontSize} px. Manual scale and offsets are preserved.`,
      textFitTruncated: (label: string, requestedFontSize: number, resolvedFontSize: number) => `Automatic fit reduced ${requestedFontSize} px to the ${resolvedFontSize} px minimum; some ${label.toLowerCase()} text will still be omitted on export.`,
      nation: "Nation",
      type: "Card type",
      rarity: "Rarity",
      set: "Set",
    ...
    canvas: {
      ...
      summaryHeading: "Readable card content",
      summaryIdentity: (kind: string, nation: string, rarity: string, set: string) => `Type: ${kind}. Nation: ${nation}. Rarity: ${rarity}. Set: ${set}.`,
      summaryCosts: (deployment: string, operation: string) => `Deployment cost: ${deployment}. Operation cost: ${operation}.`,
      summaryStats: (attack: string, defense: string, hqDefense: string) => `Attack: ${attack}. Defense: ${defense}. HQ defense: ${hqDefense}.`,
      summaryKeywords: (keywords: string) => `Keywords: ${keywords}.`,
      summaryEmpty: "none",
      keyboardAlternative: "Use the Artwork crop number fields or sliders for keyboard control.",
    },
    projectPanel: {
      aria: "Card workspace",
      heading: "Workspace",
      ...
      appearancePresetScope: "Changes serialized appearance only",
      appearancePreset: "Preset",
      applyAppearancePreset: "Apply appearance preset",
      ...
      diffHint: "Differences above the stated channel threshold are located below; levels are review signals, not pass/fail results.",
      diffReviewLevel: "Review level",
      diffReviewNone: "No detected change",
      diffReviewLimited: "Limited",
      diffReviewNoticeable: "Noticeable",
      diffReviewBroad: "Broad",
      diffThreshold: (threshold: number) => `Channel threshold: ${threshold}`,
      diffBounds: (xStart: number, xEnd: number, yStart: number, yEnd: number) => `Changed area: x ${xStart}–${xEnd}, y ${yStart}–${yEnd}`,
      diffNoBounds: "No pixels exceeded the threshold.",
      exportReady: "Ready",
      ...
      diagnosticFontsNotReady: "Fonts are still loading. Wait for final text measurement before exporting.",
      diagnosticTextMeasurementPending: "Text measurement is still updating. Wait for the current card preview before exporting.",
      diagnosticArtworkNotReady: "Artwork is still loading. Wait before exporting.",
      diagnosticTitleTruncated: "The title reached its minimum size and will be shortened with an ellipsis.",
      diagnosticBodyTruncated: "The body reached its minimum size and final visible line; remaining text will be omitted.",
      ...
      diagnosticTextLayoutChanged: "Text layout changed during export. Review the current fit warnings, then export again.",
  ...
  zh: {
    ...
    help: {
      ...
      eyebrow: "使用指南",
      title: "制作并保留你的卡牌",
      intro: "先在编辑区完成卡面，需要时使用撤销或重做，再通过工作台查找参考、保存项目或导出成品。",
      quickStart: "推荐流程",
      quickSteps: [
        {
          title: "构建卡面",
      ...
      workspaceItems: [
        { title: "外观", body: "应用五种已序列化的外观预设之一、调整纸张纹理，或加载本地风格包。" },
        { title: "卡库", body: "打开项目文件，并在所选文件夹中管理可重复使用的卡牌。" },
        { title: "导出", body: "选择格式、分辨率、图像调整和保存位置。" },
        { title: "参考", body: "搜索或使用随应用提供的参考卡，并定位与所选图片之间的视觉差异。" },
      ],
      saveTitle: "如何保留你的工作",
      saveItems: [
      ...
      tipsTitle: "常见情况",
      tips: [
        "不在文字输入框中时，使用 Ctrl/Cmd+Z 撤销，使用 Ctrl/Cmd+Shift+Z 或 Ctrl+Y 重做；文字输入框保留浏览器自己的文本撤销。",
        "标题和正文会从人工字号与缩放设置开始自动适配；每个字段下方会显示实际字号，并在达到最小字号后仍会省略文字时提醒你。",
        "Canvas 预览包含可读卡牌摘要；键盘操作请使用卡图裁剪数字输入框或滑杆。",
        "视觉差异的等级、阈值和坐标只是检查信号，不代表通过或失败。",
        "卡图加载完成后再导出。",
    ...
    fieldPanel: {
      ...
      textFitFits: (fontSize: number) => `自动适配：当前 ${fontSize} px 字号可以完整显示。`,
      textFitAdjusted: (requestedFontSize: number, resolvedFontSize: number) => `自动适配已从 ${requestedFontSize} px 缩小到 ${resolvedFontSize} px；人工缩放和偏移保持不变。`,
      textFitTruncated: (label: string, requestedFontSize: number, resolvedFontSize: number) => `自动适配已从 ${requestedFontSize} px 缩小到 ${resolvedFontSize} px 最小字号；导出时仍会省略部分${label}。`,
      nation: "阵营",
      type: "卡种",
      rarity: "稀有度",
      set: "卡包",
    ...
    canvas: {
      ...
      summaryHeading: "可读卡牌内容",
      summaryIdentity: (kind: string, nation: string, rarity: string, set: string) => `卡种：${kind}。阵营：${nation}。稀有度：${rarity}。卡包：${set}。`,
      summaryCosts: (deployment: string, operation: string) => `部署费用：${deployment}。行动费用：${operation}。`,
      summaryStats: (attack: string, defense: string, hqDefense: string) => `攻击：${attack}。防御：${defense}。总部防御：${hqDefense}。`,
      summaryKeywords: (keywords: string) => `关键词：${keywords}。`,
      summaryEmpty: "无",
      keyboardAlternative: "键盘操作请使用“卡图”区域的裁剪数字输入框或滑杆。",
    },
    projectPanel: {
      aria: "卡牌工作台",
      heading: "工作台",
      ...
      appearancePresetScope: "只修改已序列化的外观设置",
      appearancePreset: "预设",
      applyAppearancePreset: "应用外观预设",
      ...
      overallDiff: "综合差异",
      changed: "变化像素占比",
      diffHint: "下方会定位超过通道阈值的差异；等级只用于辅助检查，不代表通过或失败。",
      diffReviewLevel: "检查等级",
      diffReviewNone: "未检测到变化",
      diffReviewLimited: "少量",
      diffReviewNoticeable: "明显",
      diffReviewBroad: "广泛",
      diffThreshold: (threshold: number) => `通道阈值：${threshold}`,
      diffBounds: (xStart: number, xEnd: number, yStart: number, yEnd: number) => `变化区域：x ${xStart}–${xEnd}，y ${yStart}–${yEnd}`,
      diffNoBounds: "没有像素超过阈值。",
      exportReady: "可以导出",
      exportWarning: "请先检查",
      ...
      diagnosticFontsNotReady: "字体仍在加载，请等待最终文字测量完成后再导出。",
      diagnosticTextMeasurementPending: "文字测量仍在更新，请等待当前卡牌预览完成后再导出。",
      diagnosticArtworkNotReady: "卡图仍在加载，请稍候再导出。",
      diagnosticTitleTruncated: "标题已达到最小字号，超出部分会以省略号缩短。",
      diagnosticBodyTruncated: "正文已达到最小字号和可见行数，剩余内容不会出现在导出文件中。",
      diagnosticAssetPackWarning: "风格包提醒：",
      diagnosticProgramTexture: "本次将使用生成的纸张纹理。",
      diagnosticPrivateConfirmation: "导出前需要确认使用本地风格包。",
      diagnosticTextLayoutChanged: "导出时文字排版发生变化，请检查当前适配提醒后再次导出。",
